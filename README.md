...
Because it can be grouped into four blocks of 64 values, aligned at
...
60 OP_RUN values, or vice versa.
...
and my next step was replacing bitmask checks with
`>=` checks, and bit masks with subtractions.
...
else if (code >= 128) { decodeOpRGB565(); }
else if (code >= 124) { decodeOpLuma(code - 124); }
else if ...
```

It may seem that a downside of this is that you can't efficiently check
for an arbitrary OP like you can with bitmasks, but that point is moot.
...
The logical choice for this ordering is the frequency with
...
This is the final code map for FluidQOI (back to the 32BPP version):
...
- Encoder statistics of how many OP codes are used, how many of the values
  (first bytes of the OP codes) are used, and how many of which deltas occur.
  All of this helps decide the order of the OP
  codes and which DELTA/LUMA variant(s) to employ.
